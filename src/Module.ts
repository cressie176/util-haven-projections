...
export default class Module {
  ...
  publish({ dryRun = false } : { dryRun: boolean }) {
    debug(`Publishing module ${this._name}@${this._version} from ${this._baseDir} with dryRun=${dryRun}`);
    npm.publish({ cwd: this._baseDir, dryRun });
  }

  link() {
    debug(`Linking module ${this._name}@${this._version} from ${this._baseDir}`);
  ...
  _init() {
    ...
    fs.writeFileSync(path.join(this._baseDir, `types.d.ts`), this._types, 'utf-8');
  }

  _getCurrentAndFutureRecords() {
    const now = new Date();
    const startDate = this._records.map(({ effectiveDate }) => effectiveDate).reduce((winner, candidate) => {
      if (!winner) return candidate;
      if (candidate > winner && candidate <= now) return candidate;
      if (candidate < winner && candidate >= now) return candidate;
      return winner;
    });
    return this._records.filter(({ effectiveDate }) => effectiveDate >= startDate);
  };
